...
ArgSpec specs[] = {
    ...
    {'\0', "ttl-list", __args_set_field_ttl_list, false, "L[N]",
     ...
     true},
    {'\0', "ttl-mix", __args_set_field_ttl_mix, false, "L[N]",
     "Set weights for list of lifetimes", "Block lifetime", NULL, 0,
     arg_intlist, true},

    {'t', "touch-stride", __args_set_field_touch_stride, false, "BYTES",
     "Step between writes when \"warming\" a block", "Touch/fault parameters",
     NULL, 0, arg_size(4096u), true},
...
static void check_args(Args *args, Int iter) {
    ...
    if (((args->ttl_list.as.il.count > 0) && (args->ttl_mix.as.il.count > 0)) &&
        (args->ttl_list.as.il.count != args->ttl_mix.as.il.count)) {
        fatal("Number of weights doesn't match to the number of items in "
              "lifetime list. W(%u) != L(%u)",
              args->ttl_mix.as.il.count, args->ttl_list.as.il.count);
    }
...
void log_args(const Args *args) {
    ...
    log_debug("args.ttl_mix = %s", str_int_list(&args->ttl_mix.as.il));

    log_debug("args.touch_stride = %zu", args->touch_stride.as.i);

    log_debug("args.snap_interval = %zu", args->snap_interval.as.i);
    log_debug("args.output = %s", args->output.as.s);
    log_debug("args.display = %d", args->verbose.as.b);
}
