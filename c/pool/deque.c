...
#define LOG_DEL_BLOCK(block)                                                   \
    log_info(ANSI_COLOR(RED, "Deleting") "   " DBG_BLOCK_STR,                  \
             size_str((block).size, 2), (block).ttl_org)
...
static Block *deque_add(Deque *deq, usize size, isize ttl,
    ...
    if (deq->head == NULL) {
        deq->head = dn;
        deq->tail = dn;
    } else if (DEQUE_APPEND) {
        deq->tail->next = dn;
        dn->prev = deq->tail;
        deq->tail = dn;
    } else if (DEQUE_PREPEND) {
        deq->head->prev = dn;
        dn->next = deq->head;
        deq->head = dn;
    } else {
...
static void deque_remove(Deque *deq, usize idx, DequeNode *node,
    ...
    case DEQUE_POP_IDX: {
        ...
        if ((idx > 0) && (idx < (deq->count - 1))) {
            ...
            assert_neq(dn->prev, NULL);
            assert_neq(dn->next, NULL);
