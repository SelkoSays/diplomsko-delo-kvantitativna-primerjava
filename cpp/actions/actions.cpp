...
static SInt get_block_ttl(const Args &args, Random &rng) {
    ...
    case TTL_LIST:
        return (SInt)rng.choice(args.ttl_list.as.il, args.ttl_mix.as.il);
    default:
...
void block_action(Pool &pool, const Args &args, Random &rng) {
    ...
    if (!alloc) {
        ...
        log_warn("Block size: %lu", block_size);
        SInt block_ttl = get_block_ttl(args, rng);
        log_info(ANSI_COLOR(GREEN, "Allocating") " " DBG_BLOCK_STR,
                 size_str(block_size, 2), block_ttl);
        Block &block = pool.add_block(block_size, block_ttl);
        pool.touch_pages(block, args.touch_stride.as.i);
    }
}
