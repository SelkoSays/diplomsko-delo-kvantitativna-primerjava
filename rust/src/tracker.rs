...
unsafe impl<A: GlobalAlloc> GlobalAlloc for TrackingAllocator<A> {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if !ptr.is_null() {
            unsafe {
                GLOBAL_TRACKER.add_alloc(layout.size());
            }
        }
        ptr
    }
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        if !ptr.is_null() {
            unsafe {
                GLOBAL_TRACKER.remove_alloc(layout.size());
            }
        }
        unsafe {
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        ...
        if !new_ptr.is_null() {
            unsafe {
                GLOBAL_TRACKER.add_alloc(new_size);
            }
        }
        new_ptr
    }
...
// #[global_allocator]
// pub static GLOBAL: TrackingAllocator<System> = TrackingAllocator::new();
