...
public final class ArgsParser {
    ...
    public static final class Args {
        ...
        public Arg ttl_mix = Arg.intListArg(new ArrayList<>());

        // Touch/fault parameters
        public Arg touch_stride = Arg.sizeArg(4096);
        ...
        public String toString() {
            ...
            sb.append("  ttl_mix: ").append(ttl_mix).append(",\n");
            sb.append("  touch_stride: ").append(touch_stride).append(",\n");
    ...
    private static ArgSpecs buildSpecs(Args args) {
        ...
        specs.add(new ArgSpec('\0', "ttl-mix", v -> args.ttl_mix = v, false, "L[N]",
                "Set weights for list of lifetimes", null, Arg.intListArg(new ArrayList<>()), true));

        // Touch/fault parameters
        specs.changeGroup("Touch/fault parameters");
        specs.add(new ArgSpec('t', "touch-stride", v -> args.touch_stride = v, false, "BYTES",
                "Step between writes when \"warming\" a block", null, Arg.sizeArg(4096), true));
    ...
    private static void checkArgs(Args args, long iterDefault) {
        ...
        if (!args.ttl_list.asIntList().isEmpty() && !args.ttl_mix.asIntList().isEmpty() &&
                args.ttl_list.asIntList().size() != args.ttl_mix.asIntList().size()) {
            throw new IllegalArgumentException(String.format(
                    "Number of weights doesn't match the number of items in lifetime list. W(%d) != L(%d)",
                    args.ttl_mix.asIntList().size(), args.ttl_list.asIntList().size()));
        }
    ...
    public static void logArgs(Args args) {
        ...
        System.out.println("args.ttl_mix = " + args.ttl_mix);
        System.out.println("args.touch_stride = " + args.touch_stride.asSize());
        System.out.println("args.snap_interval = " + args.snap_interval.asInt());
        System.out.println("args.output = " + args.output.asStr());
        System.out.println("args.display = " + args.verbose.asBool());
    }
}
